...
WITH movie_diversity AS (
    SELECT
        m.movie_id,
        COUNT(DISTINCT bg.genre_id) AS genre_diversity,
        COUNT(DISTINCT bl.language_id) AS language_diversity
...
movie_ratings AS (
    SELECT
        ...
        -- ✅ fix: ignore fake 0 ratings
        NULLIF(f.avg_rating, 0) AS avg_rating,

        f.revenue_clean,
        f.budget_clean
    FROM {{ ref('fact_movie_metrics') }} f
),
...
final AS (
    SELECT
        *,
        ...
        (diversity_score_raw * avg_rating) AS diversity_index,

        {{ profitability_flag('revenue_clean', 'budget_clean') }} AS profitability_flag,
        {{ language_type('language_diversity') }} AS language_type,

        CASE 
            WHEN diversity_score_raw >= (
                SELECT AVG(diversity_score_raw) FROM combined
            )
            THEN 'High'
            ELSE 'Low'
        ...
        RANK() OVER (
            ORDER BY (diversity_score_raw * avg_rating) DESC
        ) AS diversity_rank
